...
import com.haoweifukai.LiuDongRenKouCaijiGuanLi.adapter.BlackListAdapter;
...
public class BlackListActivity extends PermanentActivity
        implements AdapterView.OnItemClickListener, View.OnClickListener {

    private static String TAG = BlackListActivity.class.getSimpleName();
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        ...
        intent.putExtra(AlertDetailActivity.SELECTED_PERSON_ID, data.id);
        startActivity(intent);
        overridePendingTransition(R.anim.anim_in, R.anim.anim_out);
        new MyContentProvider().addSystemLog(mOwnerData, "信息检索", "预警信息", String.valueOf(data.id), "检索预警信息", "民警");
    }
    ...
    protected void initViews() {
        ...
        mAdapter = new BlackListAdapter(this, mDataList);
    ...
    private void loadData() {
        ...
        Cursor cursor = contentProvider.query(YuJingColumns.CONTENT_URI, null, null, null, null);
        ...
        if (cursor != null) {
            ...
            if (mRecordCount > 0 && cursor.moveToPosition(mPageIndex * Config.ONCE_LOAD_RECORD_COUNT)) {
                ...
                do {
                    ...
                    data.id = cursor.getString(cursor.getColumnIndex(YuJingColumns.YJXXID));
                    data.card_id = cursor.getString(cursor.getColumnIndex(YuJingColumns.BDSFZH));

                    Cursor ryCursor = contentProvider.query(LaiJingRenYuanColumns.CONTENT_URI, null,
                            LaiJingRenYuanColumns.RYID + "='" + cursor.getString(cursor.getColumnIndex(YuJingColumns.LJRYRYID)) + "'",
                            null, null);
                    if (ryCursor != null) {
                        if (ryCursor.getCount() > 0 && ryCursor.moveToFirst()) {
                            data.name = ryCursor.getString(ryCursor.getColumnIndex(LaiJingRenYuanColumns.XM));
                        }
    ...
    private class PrepareImportDataTask extends AsyncTask<File, Integer, XLSUtil.XLSFileInfo> {
        ...
        protected void onPreExecute() {
            mProgressDialog = new ProgressDialog(BlackListActivity.this);
            ...
            if (result.isXLS) {
                if (result.tableIndex == XLSUtil.HEIMINGDAN_MD5_TABLE_INDEX) {
                    if (result.recordCount > 0)
                        ...
                        CommonUtils.createErrorAlertDialog(BlackListActivity.this, "无纪录！").show();
                } else {
                    CommonUtils.createErrorAlertDialog(BlackListActivity.this, "不是黑名单XLS文件！").show();
                }
            } else {
                CommonUtils.createErrorAlertDialog(BlackListActivity.this, "文件格式不正确！").show();
            }
        }
    }
    ...
    private class ImportDataTask extends AsyncTask<File, Integer, Boolean> {
        ...
        protected void onPostExecute(Boolean success) {
            ...
            onResume();
            CommonUtils.createErrorAlertDialog(BlackListActivity.this, "成功导入完成").show();
        }
        ...
        protected Boolean doInBackground(File... params) {
            boolean success = XLSUtil.importXLS(BlackListActivity.this, params[0], mTableIndex, new ProgressUpdateListener() {
                @Override
